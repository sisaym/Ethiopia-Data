...
def import_search(request):
    ...
    context_dict['form'] = form
    if request.method == 'POST':
        year = request.POST['year'] if request.POST['year'] != None else None
        origin = request.POST['origin'] if request.POST['origin']!= None else None
        item = request.POST['item'] if request.POST['item']!= None else None

        if len(year)>1 and len(item)==len(origin)==0:
            imports = Import.objects.filter(year__year_code=year)
            imports = imports.values('origin').annotate(cif_Value_in_usd=Sum(
            'cif_Value_in_usd'),Volume_in_tons=Sum('Volume_in_tons'),
                                                         cif_Value_in_etb=Sum('cif_Value_in_etb')).order_by('-cif_Value_in_usd')
            context_dict['data'] = imports
            context_dict['warning'] = "No data man"
